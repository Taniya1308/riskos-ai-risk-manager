...
export default function RiskDetailsModal({
  ...
  const [activeTab, setActiveTab] = useState<'investigation' | 'signals' | 'notes' | 'audit' | 'chat'>('investigation');
    ...
    async function loadData() {
      ...
      try {
        const [res, notesRes] = await Promise.all([
          fetch(`/api/audit-logs?caseId=${(caseData as Record<string,unknown>).id}`),
          fetch(`/api/cases/notes?caseId=${(caseData as Record<string,unknown>).id}`),
        ]);
  ...
  const TABS = [
    ...
    { id: 'audit',         label: 'History',       icon: Lock },
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs font-semibold text-slate-300">Analyst Notes</p>
                </div>
                <span className="text-[11px] text-slate-600">{notes.length} note{notes.length !== 1 ? 's' : ''}</span>
              </div>
              ...
              <div className="space-y-2">
                <textarea
                  ...
                  placeholder="Add a note about this case — observations, context, or reasoning for your decision…"
                  rows={3}
                  className="w-full bg-slate-800/60 border border-slate-700 rounded-xl px-4 py-3 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 resize-none transition"
                />
                <button
              ...
              {notes.length === 0 ? (
                <div className="flex flex-col items-center gap-2 py-10 text-center">
                  ...
                  <p className="text-xs text-slate-700">Add observations or context before making your decision</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {notes.map(note => (
